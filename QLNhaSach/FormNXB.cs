    ...
    public partial class FormNXB : Form
    {
        ...
        void LoadData()
        {
            try
            {
                ...
                dataGridView1.DataSource = db.LayNXB();
            ...
            {
                MessageBox.Show("Không lấy được nội dung trong table NXB. Lỗi rồi!!!");
            }
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                this.txtMaNXB.Text = dataGridView1.Rows[r].Cells[0].Value.ToString();
                this.txtTenNXB.Text = dataGridView1.Rows[r].Cells[1].Value.ToString();
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            if (them)
            {
                try
                {
                    ...
                    bl.ThemNXB(this.txtMaNXB.Text.Trim(), this.txtTenNXB.Text.Trim(), this.txtDiaChi.Text.Trim());
            ...
            else
            {
                // Thực hiện lệnh
                BLInfomation bl = new BLInfomation();
                bl.CapNhatNXB(this.txtMaNXB.Text.Trim(), this.txtTenNXB.Text.Trim(), this.txtDiaChi.Text.Trim());
                // Load lại dữ liệu trên DataGridView
                LoadData();
                // Thông báo
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            if (txtSearch.Text != "")
            {
                if (cLogin.search == 0)
                {
                    dataGridView1.DataSource = db.SearchNXBMaNXB(txtSearch.Text);
                }
                else if (cLogin.search == 1)
                {
                    dataGridView1.DataSource = db.SearchNXBTenNXB(txtSearch.Text);
                }
                else
                {
                    dataGridView1.DataSource = db.SearchNXBDiaChi(txtSearch.Text);
                }
            }
            else LoadData();
        }
        ...
        private void cmbSearch_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                case "Mã NXB": cLogin.search = 0; break;
                case "Tên NXB": cLogin.search = 1; break;
                case "Địa chỉ": cLogin.search = 2;break;
            }
        }
    }
}
